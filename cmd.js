...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        ...
        try {
            ...
            if (!command.config || !command.run || !command.config.commandCategory)
                throw new Error('Module has invalid format!');
    ...
    if (errorList.length !== 0) {
        api.sendMessage('Some modules encountered errors while loading: ' + errorList.join(' '), threadID, messageID);
    }
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (command) {
        ...
        case "info": {
            ...
            if (!commandInfo) return api.sendMessage("The module you entered does not exist!", threadID, messageID);

            const { name, version, hasPermission, credits, cooldowns, dependencies } = commandInfo.config;

            return api.sendMessage(
                "=== " + name.toUpperCase() + " ===\n" +
                "- Coded by: " + credits + "\n" +
                "- Version: " + version + "\n" +
                "- Required Permission: " + ((hasPermission === 0) ? "User" : (hasPermission === 1) ? "Admin" : "Bot Operator") + "\n" +
